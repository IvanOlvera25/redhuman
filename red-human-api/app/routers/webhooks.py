...
async def whatsapp_entrante(request: Request, db: Session = Depends(get_db)):
    ...
    Este endpoint es público, así que lo primero es verificar que el POST venga
    de Meta: se valida la firma X-Hub-Signature-256 con META_APP_SECRET sobre el
    cuerpo crudo, antes de interpretar nada.
    """
    ...
    # Meta reintenta el webhook si tardamos en contestar (el prefiltro llama a
    # OpenAI y puede tardar segundos): no procesar dos veces el mismo mensaje.
    wamid = msg.get("wa_id", "")
    ...
    # Adjuntos (foto, PDF, audio): se registran en la conversación, pero el
    # agente de texto no los interpreta.
    if not texto:
        db.add(Mensaje(
            candidato_id=c.id,
            rol="user",
        ...
        aviso = "Recibí tu archivo 📎 y quedó en tu expediente. Si quieres, cuéntame por texto en qué te apoyo."
        envio = await enviar_mensaje(telefono, aviso)
        db.add(Mensaje(candidato_id=c.id, rol="assistant", texto=aviso, canal="whatsapp", enviado=envio.get("enviado", False)))
        db.commit()
